...
using PirateNationContracts.PopForwarder_Proxy.ContractDefinition;

namespace PirateNationContracts.PopForwarder_Proxy
{
    public partial class PopforwarderProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, PopforwarderProxyDeployment popforwarderProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<PopforwarderProxyDeployment>().SendRequestAndWaitForReceiptAsync(popforwarderProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, PopforwarderProxyDeployment popforwarderProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<PopforwarderProxyDeployment>().SendRequestAsync(popforwarderProxyDeployment);
        }

        public static async Task<PopforwarderProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, PopforwarderProxyDeployment popforwarderProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, popforwarderProxyDeployment, cancellationTokenSource);
            return new PopforwarderProxyService(web3, receipt.ContractAddress);
        }
        ...
        public PopforwarderProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
