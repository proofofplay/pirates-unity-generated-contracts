...
using PirateNationContracts.AffinitySystem_Proxy.ContractDefinition;

namespace PirateNationContracts.AffinitySystem_Proxy
{
    public partial class AffinitysystemProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, AffinitysystemProxyDeployment affinitysystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<AffinitysystemProxyDeployment>().SendRequestAndWaitForReceiptAsync(affinitysystemProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, AffinitysystemProxyDeployment affinitysystemProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<AffinitysystemProxyDeployment>().SendRequestAsync(affinitysystemProxyDeployment);
        }

        public static async Task<AffinitysystemProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, AffinitysystemProxyDeployment affinitysystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, affinitysystemProxyDeployment, cancellationTokenSource);
            return new AffinitysystemProxyService(web3, receipt.ContractAddress);
        }
        ...
        public AffinitysystemProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
