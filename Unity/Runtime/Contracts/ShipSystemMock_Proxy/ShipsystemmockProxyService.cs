...
using PirateNationContracts.ShipSystemMock_Proxy.ContractDefinition;

namespace PirateNationContracts.ShipSystemMock_Proxy
{
    public partial class ShipsystemmockProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, ShipsystemmockProxyDeployment shipsystemmockProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<ShipsystemmockProxyDeployment>().SendRequestAndWaitForReceiptAsync(shipsystemmockProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, ShipsystemmockProxyDeployment shipsystemmockProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<ShipsystemmockProxyDeployment>().SendRequestAsync(shipsystemmockProxyDeployment);
        }

        public static async Task<ShipsystemmockProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, ShipsystemmockProxyDeployment shipsystemmockProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, shipsystemmockProxyDeployment, cancellationTokenSource);
            return new ShipsystemmockProxyService(web3, receipt.ContractAddress);
        }
        ...
        public ShipsystemmockProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
