...
using PirateNationContracts.PopRandomizer.ContractDefinition;

namespace PirateNationContracts.PopRandomizer
{
    public partial class PopRandomizerService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, PopRandomizerDeployment popRandomizerDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<PopRandomizerDeployment>().SendRequestAndWaitForReceiptAsync(popRandomizerDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, PopRandomizerDeployment popRandomizerDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<PopRandomizerDeployment>().SendRequestAsync(popRandomizerDeployment);
        }

        public static async Task<PopRandomizerService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, PopRandomizerDeployment popRandomizerDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, popRandomizerDeployment, cancellationTokenSource);
            return new PopRandomizerService(web3, receipt.ContractAddress);
        }
        ...
        public PopRandomizerService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
        ...
        public Task<string> RecievedRandomNumberRequestAsync(RecievedRandomNumberFunction recievedRandomNumberFunction)
        {
             return ContractHandler.SendRequestAsync(recievedRandomNumberFunction);
        }

        public Task<TransactionReceipt> RecievedRandomNumberRequestAndWaitForReceiptAsync(RecievedRandomNumberFunction recievedRandomNumberFunction, CancellationTokenSource cancellationToken = null)
        {
             return ContractHandler.SendRequestAndWaitForReceiptAsync(recievedRandomNumberFunction, cancellationToken);
        }

        public Task<string> RecievedRandomNumberRequestAsync(BigInteger requestId, BigInteger randomNumber)
        {
            var recievedRandomNumberFunction = new RecievedRandomNumberFunction();
                recievedRandomNumberFunction.RequestId = requestId;
                recievedRandomNumberFunction.RandomNumber = randomNumber;
            
             return ContractHandler.SendRequestAsync(recievedRandomNumberFunction);
        }

        public Task<TransactionReceipt> RecievedRandomNumberRequestAndWaitForReceiptAsync(BigInteger requestId, BigInteger randomNumber, CancellationTokenSource cancellationToken = null)
        {
            var recievedRandomNumberFunction = new RecievedRandomNumberFunction();
                recievedRandomNumberFunction.RequestId = requestId;
                recievedRandomNumberFunction.RandomNumber = randomNumber;
