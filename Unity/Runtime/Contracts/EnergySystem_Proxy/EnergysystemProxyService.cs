...
using PirateNationContracts.EnergySystem_Proxy.ContractDefinition;

namespace PirateNationContracts.EnergySystem_Proxy
{
    public partial class EnergysystemProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, EnergysystemProxyDeployment energysystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<EnergysystemProxyDeployment>().SendRequestAndWaitForReceiptAsync(energysystemProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, EnergysystemProxyDeployment energysystemProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<EnergysystemProxyDeployment>().SendRequestAsync(energysystemProxyDeployment);
        }

        public static async Task<EnergysystemProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, EnergysystemProxyDeployment energysystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, energysystemProxyDeployment, cancellationTokenSource);
            return new EnergysystemProxyService(web3, receipt.ContractAddress);
        }
        ...
        public EnergysystemProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
