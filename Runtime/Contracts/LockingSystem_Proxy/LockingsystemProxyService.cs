...
using PirateNationContracts.LockingSystem_Proxy.ContractDefinition;

namespace PirateNationContracts.LockingSystem_Proxy
{
    public partial class LockingsystemProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, LockingsystemProxyDeployment lockingsystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<LockingsystemProxyDeployment>().SendRequestAndWaitForReceiptAsync(lockingsystemProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, LockingsystemProxyDeployment lockingsystemProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<LockingsystemProxyDeployment>().SendRequestAsync(lockingsystemProxyDeployment);
        }

        public static async Task<LockingsystemProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, LockingsystemProxyDeployment lockingsystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, lockingsystemProxyDeployment, cancellationTokenSource);
            return new LockingsystemProxyService(web3, receipt.ContractAddress);
        }
        ...
        public LockingsystemProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
