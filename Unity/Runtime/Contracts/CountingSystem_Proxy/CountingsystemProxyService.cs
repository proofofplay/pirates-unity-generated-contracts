...
using PirateNationContracts.CountingSystem_Proxy.ContractDefinition;

namespace PirateNationContracts.CountingSystem_Proxy
{
    public partial class CountingsystemProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, CountingsystemProxyDeployment countingsystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<CountingsystemProxyDeployment>().SendRequestAndWaitForReceiptAsync(countingsystemProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, CountingsystemProxyDeployment countingsystemProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<CountingsystemProxyDeployment>().SendRequestAsync(countingsystemProxyDeployment);
        }

        public static async Task<CountingsystemProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, CountingsystemProxyDeployment countingsystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, countingsystemProxyDeployment, cancellationTokenSource);
            return new CountingsystemProxyService(web3, receipt.ContractAddress);
        }
        ...
        public CountingsystemProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
