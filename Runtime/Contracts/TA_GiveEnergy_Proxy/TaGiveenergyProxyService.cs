...
using PirateNationContracts.TA_GiveEnergy_Proxy.ContractDefinition;

namespace PirateNationContracts.TA_GiveEnergy_Proxy
{
    public partial class TaGiveenergyProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, TaGiveenergyProxyDeployment taGiveenergyProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<TaGiveenergyProxyDeployment>().SendRequestAndWaitForReceiptAsync(taGiveenergyProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, TaGiveenergyProxyDeployment taGiveenergyProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<TaGiveenergyProxyDeployment>().SendRequestAsync(taGiveenergyProxyDeployment);
        }

        public static async Task<TaGiveenergyProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, TaGiveenergyProxyDeployment taGiveenergyProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, taGiveenergyProxyDeployment, cancellationTokenSource);
            return new TaGiveenergyProxyService(web3, receipt.ContractAddress);
        }
        ...
        public TaGiveenergyProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
