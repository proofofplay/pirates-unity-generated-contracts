...
using PirateNationContracts.CaptainSystem_Proxy.ContractDefinition;

namespace PirateNationContracts.CaptainSystem_Proxy
{
    public partial class CaptainsystemProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, CaptainsystemProxyDeployment captainsystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<CaptainsystemProxyDeployment>().SendRequestAndWaitForReceiptAsync(captainsystemProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, CaptainsystemProxyDeployment captainsystemProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<CaptainsystemProxyDeployment>().SendRequestAsync(captainsystemProxyDeployment);
        }

        public static async Task<CaptainsystemProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, CaptainsystemProxyDeployment captainsystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, captainsystemProxyDeployment, cancellationTokenSource);
            return new CaptainsystemProxyService(web3, receipt.ContractAddress);
        }
        ...
        public CaptainsystemProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
