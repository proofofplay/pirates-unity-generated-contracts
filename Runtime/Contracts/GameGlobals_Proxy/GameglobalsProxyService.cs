...
using PirateNationContracts.GameGlobals_Proxy.ContractDefinition;

namespace PirateNationContracts.GameGlobals_Proxy
{
    public partial class GameglobalsProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, GameglobalsProxyDeployment gameglobalsProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<GameglobalsProxyDeployment>().SendRequestAndWaitForReceiptAsync(gameglobalsProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, GameglobalsProxyDeployment gameglobalsProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<GameglobalsProxyDeployment>().SendRequestAsync(gameglobalsProxyDeployment);
        }

        public static async Task<GameglobalsProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, GameglobalsProxyDeployment gameglobalsProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, gameglobalsProxyDeployment, cancellationTokenSource);
            return new GameglobalsProxyService(web3, receipt.ContractAddress);
        }
        ...
        public GameglobalsProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
