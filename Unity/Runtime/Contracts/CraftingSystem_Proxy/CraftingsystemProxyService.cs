...
using PirateNationContracts.CraftingSystem_Proxy.ContractDefinition;

namespace PirateNationContracts.CraftingSystem_Proxy
{
    public partial class CraftingsystemProxyService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, CraftingsystemProxyDeployment craftingsystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<CraftingsystemProxyDeployment>().SendRequestAndWaitForReceiptAsync(craftingsystemProxyDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, CraftingsystemProxyDeployment craftingsystemProxyDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<CraftingsystemProxyDeployment>().SendRequestAsync(craftingsystemProxyDeployment);
        }

        public static async Task<CraftingsystemProxyService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, CraftingsystemProxyDeployment craftingsystemProxyDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, craftingsystemProxyDeployment, cancellationTokenSource);
            return new CraftingsystemProxyService(web3, receipt.ContractAddress);
        }
        ...
        public CraftingsystemProxyService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
